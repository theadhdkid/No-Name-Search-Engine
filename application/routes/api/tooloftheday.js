...
export default async function (fastify, opts) {
   ...
   */
  async function getDailyTool() {
    // Gets whole of daily tool
    ...
    fastify.log.error(
      "Failed to select a daily AITool despite items being present. Index:",
      skipIndex,
    );
    return null;
  }
  ...
  fastify.get("/api/tooloftheday", async (request, reply) => {
    try {
      ...
      if (dailyTool) {
        // Return entire tool object directly
        ...
        reply
          ...
          .send({
            error:
              "Could not determine the tool of the day. No AI tools found or selection failed.",
          });
      }
    } catch (error) {
      ...
      reply
        .code(500)
        .send({
          error: "Internal Server Error while fetching tool of the day.",
        });
    }
  });
}
